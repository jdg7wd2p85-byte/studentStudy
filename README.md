...
- 梦想模块：用数据库保存孩子的目标和愿望，手机和电脑可同步查看
- 火箭发射小游戏：可调推力和重力，支持两级分离、每一级回收、燃料和得分
- 支持科目、类别、来源、标签
- 支持批量粘贴解析
...
- 中考真题按年份、科目保留原卷链接，逐题标注知识点、年级和学习进度，记录每次作答
...
进入 `中考真题`，先选孩子，再添加一张原卷，填写年份、科目和原卷 PDF 或网页链接。打开原卷后逐题添加题号、知识点、对应年级和学习进度；题目全文可选填。原卷与答案链接在卷详情里打开。

切到 `全部年份` 可跨卷按年份、科目、年级、学习进度、最新作答结果和知识点筛选。点每道题的 `记录作答 / 历史` 保存本次结果并查看历次时间与错误原因。`未学`只表示尚未学习，不计入错题。点 `复制当前链接` 可分享当前筛选视图，例如 `/?tab=exams&grade=七年级&stage=LEARNED`。

目前真题资料由用户自行录入链接和逐题标注；网站不会自动下载或发布第三方试卷。真题数据独立于普通学习项和单词选择题。
...
进入 `梦想` 页，可以记录孩子说过的目标和画面，例如 `想考270分，以后买大别墅，在阳台装空调给猫`。当前版本已保存到数据库，并支持填写目标分数和目标日期。

### 火箭

进入 `火箭` 页，可以玩一个轻量物理实验：调整推力和重力，观察火箭上升，点击 `分离一级 / 分离二级` 体现质量变化，再分别回收每一级。页面会显示燃料、最高高度、已回收层数和得分。
