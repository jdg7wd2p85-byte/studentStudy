...
function addDream() {
  ...
  localStorage.setItem("student-study-dreams", JSON.stringify(dreams.slice(0, 30)));
  $("dreamInput").value = "";
  renderDreams();
}
...
function loadDreams() {
  try {
    return JSON.parse(localStorage.getItem("student-study-dreams") || "[]");
  } catch (error) {
    return [];
  }
}
...
function renderDreams() {
  ...
  $("dreamList").innerHTML = dreams.length ? dreams.map((dream) => `
    <article class="dream-item">
      <strong>${escapeHtml(formatDate(dream.createdAt))}</strong>
...
function launchRocket() {
  ...
  state.rocket.recovering = false;
  drawRocket();
}

function separateStage() {
  state.rocket.staged = true;
  $("rocketStatus").textContent = "一级已分离，质量变轻，继续观察速度变化。";
  drawRocket();
}

function recoverRocket() {
  state.rocket.recovering = true;
  $("rocketStatus").textContent = "开始回收：降低推力，准备软着陆。";
  drawRocket();
}
...
function resetRocket() {
  ...
  $("rocketStatus").textContent = "调整推力和重力，点击发射。推力大于重力时火箭上升。";
  drawRocketFrame();
}
...
function drawRocket() {
  ...
  const step = () => {
    ...
    const massBonus = state.rocket.staged ? 1.25 : 1;
    const recoveryDrag = state.rocket.recovering ? 0.72 : 1;
    const acceleration = ((thrust * massBonus * recoveryDrag) - gravity) * 0.018;
    if (state.rocket.running) {
      state.rocket.velocity -= acceleration;
      state.rocket.velocity *= state.rocket.recovering ? 0.94 : 0.985;
      ...
      if (state.rocket.y >= 339 && state.rocket.recovering) {
        state.rocket.running = false;
        state.rocket.velocity = 0;
        $("rocketStatus").textContent = "回收完成：火箭软着陆。";
      } else {
        $("rocketStatus").textContent = `高度 ${state.rocket.altitude} m / 速度 ${Math.abs(state.rocket.velocity * 10).toFixed(1)} / ${state.rocket.staged ? "二级飞行" : "一级飞行"}`;
      }
    }
    drawRocketFrame();
    if (state.rocket.running) {
...
function drawRocketFrame() {
  ...
  ctx.fillStyle = state.rocket.staged ? "#3d8aa8" : "#3f8f62";
  ctx.beginPath();
  ctx.moveTo(x, y - 34);
  ctx.lineTo(x - 16, y - 2);
  ctx.lineTo(x + 16, y - 2);
  ...
  if (state.rocket.running && !state.rocket.recovering) {
  ...
  if (state.rocket.recovering) {
    ctx.strokeStyle = "#7a6bb4";
    ctx.beginPath();
    ctx.arc(x, y - 38, 28, Math.PI, 0);
    ctx.stroke();
  }
}
